...
class TestViews(TestCase):
    ...
    def test_create_product(self):
        response = self.client.post(
            "/api/v1/products", json={'name': 'Netflix'})
        product = response.json
    ...
    def test_create_product_bad_request(self):
        ...
        response_2 = self.client.post(
            "/api/v1/products", json={'other': 'what'})
        product_2 = response_2.json
    ...
    def test_update_product(self):
        update_response = self.client.patch(
            "/api/v1/products/1", json={'name': 'Netlify'})
    ...
    def test_update_product_not_found(self):
        response = self.client.patch(
            "/api/v1/products/20", json={'name': 'Doctolib'})
        product = response.json
        self.assertEqual(response.status_code, 404)
        self.assertIsNone(product)
    ...
    def test_update_product_bad_request(self):
        response_1 = self.client.patch(
            "/api/v1/products/1", json={'other': 'what'})
        product_1 = response_1.json
        self.assertEqual(response_1.status_code, 400)
        self.assertIsNone(product_1)
